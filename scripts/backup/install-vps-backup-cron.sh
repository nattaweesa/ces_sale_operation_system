...
VPS_SCRIPTS_DIR="${VPS_SCRIPTS_DIR:-/root/backup-scripts}"
...
ssh "${VPS_USER}@${VPS_HOST}" "/root/backup-scripts/ces-prod-backup.sh"
ssh "${VPS_USER}@${VPS_HOST}" "echo -n 'backup_count='; find /root/ces_sale_operation_backups/production -mindepth 1 -maxdepth 1 -type d | wc -l"
