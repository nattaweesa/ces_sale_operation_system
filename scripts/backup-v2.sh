...
if [[ -z "\$DB_CONTAINER" || -z "\$BACKEND_CONTAINER" ]]; then
  echo "V2 containers are not running; cannot create backup." >&2
  exit 1
fi
...
printf '%s\n' "db_name=\$DB_NAME" >> "\$BACKUP_DIR/metadata.env"
...
docker exec "\$DB_CONTAINER" pg_dump -U ces -d "\$DB_NAME" -Fc > "\$BACKUP_DIR/db.dump"
docker exec "\$BACKEND_CONTAINER" sh -c 'tar -czf - -C /app storage_v2' > "\$BACKUP_DIR/storage_v2.tgz"

printf '%s\n' "V2 backup created at \$BACKUP_DIR"
EOF
